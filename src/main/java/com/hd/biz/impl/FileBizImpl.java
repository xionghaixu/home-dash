...
import com.hd.model.dto.FileCategorySummaryDto;
import com.hd.model.dto.FileDetailDto;
import com.hd.model.dto.FolderPathDto;
import com.hd.model.dto.RecentUploadSummaryDto;
...
public class FileBizImpl implements FileBiz {
     ...
     * @param fileDataService 文件数据访问服务
     * @param resourceDataService 资源数据访问服务
     */
    @Autowired
    public FileBizImpl(HomeDashProperties homeDashProperties,
    ...
    public ResponseDto findByParentId(Long parentId, String sortBy, String sortOrder) {
        ...
        wrapper.eq(File::getParentId, parentId);
        wrapper.orderBy(true, "asc".equalsIgnoreCase(normalizeSortOrder(sortOrder)),
                File::getCreateTime);
    ...
    public ResponseDto getRecentUploadSummary(Integer limit) {
        ...
        // 查询各时间范围的文件
        List<File> todayFiles = queryByTimeRange(todayStart.getTime(), now.getTime());
        List<File> weekFiles = queryByTimeRange(weekStart.getTime(), now.getTime());
        List<File> monthFiles = queryByTimeRange(monthStart.getTime(), now.getTime());
        ...
        recentWrapper.last("LIMIT " + safeLimit);
        List<File> recentFiles = fileDataService.list(recentWrapper);
    ...
    public ResponseDto findFilesByCategory(String category, String sortBy, String sortOrder) {
        ...
        switch (sortField) {
            case "filename", "name" -> wrapper.orderBy(true, isAsc, File::getFileName);
            case "size" -> wrapper.orderBy(true, isAsc, File::getSize);
            case "createtime", "create_time", "createTime" -> wrapper.orderBy(true, isAsc, File::getCreateTime);
            case "updatetime", "update_time", "updateTime" -> wrapper.orderBy(true, isAsc, File::getUpdateTime);
            case "type" -> wrapper.orderBy(true, isAsc, File::getType);
            default -> wrapper.orderBy(true, isAsc, File::getCreateTime);
        }
        List<File> allFiles = fileDataService.list(wrapper);
    ...
    public ResponseDto categorySummary() {
        ...
        List<File> allFiles = fileDataService.list();
    ...
    public void moveFiles(List<Long> ids, Long targetId) {
        ...
        LambdaQueryWrapper<File> targetWrapper = new LambdaQueryWrapper<>();
        targetWrapper.eq(File::getParentId, targetId);
        Set<String> targetNames = fileDataService.list(targetWrapper).stream()
        ...
        fileDataService.lambdaUpdate()
                ...
                .set(File::getUpdateTime, updateTime)
                .update();
    ...
    private boolean isDescendant(Long targetId, Long sourceId) {
        ...
        while (!currentId.equals(File.ROOT_FILE.getId()) && depth < maxDepth) {
            ...
            File currentFile = fileDataService.getById(currentId);
    ...
    private void batchCopyCommonFiles(List<File> files, File parentFile) {
        ...
        LambdaQueryWrapper<File> targetWrapper = new LambdaQueryWrapper<>();
        targetWrapper.eq(File::getParentId, parentFile.getId());
        Set<String> targetNames = fileDataService.list(targetWrapper).stream()
                .map(File::getFileName)
                .collect(java.util.stream.Collectors.toCollection(HashSet::new));
        for (File file : files) {
        ...
        List<Resource> resources = resourceDataService.listByIds(resourceIds);
        ...
        for (Map.Entry<Long, Long> entry : copyCountByResourceId.entrySet()) {
            ...
            resourceDataService.lambdaUpdate()
                    ...
                    .set(Resource::getLink, resource.getLink() + entry.getValue().intValue())
                    .update();
        }
    ...
    public FileBiz.ResourceWrapper loadResource(Long fileId) {
        ...
        Resource dbResource = resourceDataService.getById(file.getResourceId());
        ...
        try {
            // 构建文件路径并进行安全检查
            String basePath = homeDashProperties.getBasicResourcePath();
            String resourcePath = dbResource.getPath();
            ...
            throw new DataNotFoundException(
                    String.format("文件资源访问失败 [fileId=%d, resourcePath=%s]", fileId, dbResource.getPath()));
        }
    }
    ...
    private void batchDeleteCommonFiles(List<File> files) {
        ...
        if (resourceIds.isEmpty()) {
            ...
            List<Long> fileIds = files.stream()
                    .map(File::getId)
                    .collect(java.util.stream.Collectors.toList());
            fileDataService.removeByIds(fileIds);
        ...
        if (!resourceIdsToDelete.isEmpty()) {
            ...
            for (Resource resource : resourcesToDelete) {
                java.io.File resourceFile = new java.io.File(homeDashProperties.getBasicResourcePath() +
            ...
            // 批量删除资源记录
            resourceDataService.removeByIds(new ArrayList<>(resourceIdsToDelete));
        ...
        if (!resourceLinkTargetMap.isEmpty()) {
            for (Map.Entry<Long, Integer> entry : resourceLinkTargetMap.entrySet()) {
                resourceDataService.lambdaUpdate()
                        .eq(Resource::getId, entry.getKey())
                        .set(Resource::getLink, entry.getValue())
                        .update();
            }
    ...
    private void deleteFolderIterative(File folder) {
        ...
        while (!folderStack.isEmpty()) {
            ...
            // 查询当前文件夹的所有子文件
            LambdaQueryWrapper<File> wrapper = new LambdaQueryWrapper<>();
            wrapper.eq(File::getParentId, currentFolder.getId());
            List<File> children = fileDataService.list(wrapper);
    ...
    private void deleteFolder(File file) {
        // 使用MP API查询子文件，防止并发修改导致数据不一致
        LambdaQueryWrapper<File> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(File::getParentId, file.getId());
        List<File> children = fileDataService.list(wrapper);
        ...
        fileDataService.removeByIds(Collections.singletonList(file.getId()));
    }
    ...
    public void deleteCommonFile(File file) {
        ...
        if (resource.getLink() <= 1) {
            ...
            if (resourceFile.exists() && resourceFile.delete()) {
                // 物理文件删除成功，删除资源记录
                resourceDataService.removeById(resource.getId());
    ...
    private void ensureUniqueFileName(Long parentId, String fileName, Long excludeId) {
        ...
        long count = fileDataService.count(wrapper);
        if (count > 0) {
    ...
    public ResponseDto uploadWithMD5(MultipartFile file, Long parentId) {
        ...
        try {
            ...
            // 3. 查询是否已存在相同MD5的资源（秒传检查，使用MP API）
    ...
    public ResponseDto checkFileByMD5(String md5) {
        ...
        // 查询数据库
        Resource resource = resourceDataService.lambdaQuery()
                .eq(Resource::getMd5, md5)
                .one();
    ...
    public ResponseDto verifyFileMD5(Long fileId) {
        ...
        // 构建物理文件路径
        Path filePath = Paths.get(homeDashProperties.getBasicResourcePath(), resource.getPath()).normalize();
    ...
    private Resource saveUploadFile(MultipartFile file) throws IOException {
        ...
        Path fullPath = Paths.get(homeDashProperties.getBasicResourcePath(), relativePath).normalize();
     ...
     * 按时间范围查询文件。
     */
    private List<File> queryByTimeRange(java.util.Date startDate, java.util.Date endDate) {
        LambdaQueryWrapper<File> wrapper = new LambdaQueryWrapper<>();
        wrapper.between(File::getCreateTime, startDate, endDate);
        wrapper.orderByDesc(File::getCreateTime);
     ...
     * 获取排序字段对应的方法引用。
     */
    private java.util.function.Function<File, ?> getSortColumn(String sortBy) {
        if (sortBy == null) {
        ...
        return switch (sortBy.toLowerCase()) {
            ...
            case "createtime", "create_time", "createTime" -> File::getCreateTime;
            case "updatetime", "update_time", "updateTime" -> File::getUpdateTime;
